...
## 實倉部署記錄（2026-06-12 定案）

- **趨勢艙實倉：3 口微型臺指（TMF，NT$10/點/口，合計 NT$30/點）**，跟隨 v2 `trend_target`
  （SMA200 多/空手；T−1 盤後訊號 → T 開盤執行；結算日開盤平倉、次日依訊號重進，與研究會計一致）
- 風險預算（使用者設定）：單口 NT$5 萬、合計 NT$15 萬
- 帳戶配置：原始保證金 86,700（期交所 2026/05/27 公告）+ 預算緩衝 15 萬 ≈ **NT$24 萬**
- ⚠ 誠實覆蓋率（記錄於指數 44,188 時）：15 萬 ≈ −5,000 點 ≈ 名目 −11.3%；趨勢艙歷史最差
  ≈ 名目 −18~19%（等比 ≈ 24~26 萬）→ 預算覆蓋約六成歷史最差；超出時 24 萬帳戶的
  維持保證金機制會在約 −17 萬處強制減倉。**此為使用者知情選擇（2026-06-12）**
- 四票制 / LT 未實倉（LT 依帳本規則於 2027-06-12 判定採納資格）

## 部位/風險層研究（risk_layer_research.py + risk_layer_research2.py，2026-06-12 封存）

- **#2 槓桿時機**（使用者目標：加槓桿/減碼時點）：趨勢艙 × 波動率管理槓桿
  L=clip(σ_tgt/σ, 0.5, {1.5,2.0})，對照「等平均曝險固定槓桿」。4 組 devSh 增益
  +0.04~0.07 < 門檻 +0.10 → **全數未過**。已驗證的減碼時機 = SMA200 跌破出場；
  加槓桿的誠實機制 = 固定槓桿按 MDD 預算（1.0/1.5/2.0x → dev MDD ≈ NT$45/68/90 萬/口）。
  部位層研究就此封存，不再追加設定
