...
├── signal_ledger.db        # SQLite 資料庫（append-only 帳本，雲端 workflow 不寫它）
...
- **全部輸出與結論已存永久記錄 `docs/research/`（總結見 `SUMMARY.md`）**：68 組無一通過事前門檻；兩個「差點過」者（BASIS dev 0.69、PCVOL dev 0.64）OOS 全滅。2020-2026 測試窗對所有在場策略已實質燒毀 → 此後唯一誠實的驗證貨幣是前瞻實盤記錄
...
- 每日 17:03 由 `daily_update.bat` 在 v2 之後執行，記錄四艙 target：`votes_v1` / `votes_v2` / `trend` / `lt`
...
- ⚠ 不可對帳本做 UPDATE/DELETE/REPLACE；不可改 CLEAN_START/GATE_DATE；雲端 workflow 不得寫 signal_ledger.db
